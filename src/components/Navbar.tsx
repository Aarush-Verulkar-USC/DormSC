          ...
          <div className="flex items-center gap-2">
            {currentUser ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {isDropdownOpen && (
                  ...
                  >
                    ...
                    <div className="px-4 py-3 border-b border-white/[0.06]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-white truncate">{currentUser.email?.split('@')[0]}</div>
                          <div className="text-xs text-gray-500 truncate">{currentUser.email}</div>
                        </div>
                      </div>
                    </div>

                    {/* Mobile: show Listings link */}
                    <div className="md:hidden px-1.5 pt-1.5">
                      <Link
                        href="/listings"
                        className={`flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors ${pathname.startsWith('/listings')
                          ? 'text-white bg-white/10'
                          : 'text-gray-400 hover:text-white hover:bg-white/[0.06]'
                          }`}
                      >
                    ...
                    <div className="px-1.5 py-1.5">
                      {userLinks.map(link => (
                        <Link
                          key={link.href}
                          href={link.href}
                          className={`flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors ${pathname.startsWith(link.href)
                            ? 'text-white bg-white/10'
                            : 'text-gray-400 hover:text-white hover:bg-white/[0.06]'
                            }`}
                        >
                      ...
                      {isAdmin && (
                        <Link
                          href="/admin"
                          className={`flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors ${pathname === '/admin'
                    ...
                    {/* Sign out */}
                    <div className="border-t border-white/[0.06] px-1.5 pt-1.5 pb-0.5">
                      <button
              ...
              <div className="flex items-center gap-2">
                <Link
                  href="/listings"
                  className="md:hidden px-3.5 py-1.5 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-white/[0.06] transition-colors"
                >
                  Listings
                </Link>
                <Link
